...
const updateResult = () => {
    const resultTemp = calculate(Number(sanitize(leftOperand)), operatorString, Number(sanitize(rightOperand)));
    if (isNaN(resultTemp))
        result = "err";
    else
        result = treatResult(Number(resultTemp));
};
...
const handleNegate = (e) => {
    ...
    if (operatorString) {
        ...
        if (rightOperand.indexOf("--") != -1)
            rightOperand = rightOperand.slice(2);
        result = rightOperand;
    }
    else {
        leftOperand = "-" + leftOperand;
        if (leftOperand.indexOf("--") != -1)
            leftOperand = leftOperand.slice(2);
        result = leftOperand;
    }
    track = false;
    updateOutput();
